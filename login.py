...
class LoginGUI(Frame):
    ...
    def _auth(self, uname, password):
        ...
        # old version that allows SQL injections
        # sql_statement = '''SELECT name FROM users WHERE username = '%s' and password = '%s' ''' % (uname, password)

        # new version that doesn't
        sql_statement = '''SELECT name FROM users WHERE username = ? password = ? ''' + (uname, password)

        print('About to execute the following SQL statement: \n' + sql_statement)

        # new version of executed statement that prevents SQL injection
        cursor.execute(f'SELECT name FROM users WHERE username = ? and password = ? ', (uname, password))   # Execute the SQL statement we created
